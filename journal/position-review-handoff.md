...
## 2026-09-05 ~16:25 UTC · 补充轮（用户质询 live 胜率）

**本轮做了**：
- LIVE 体检入账本：胜率 37%（非用户目测的十几）、净 +$12、盈亏比 2.33 结构健康；真问题=亏损集中于"probe 追入→90 分钟内死+硬扛大止损"
- **改 decider 出场策略段（ee0675e 已部署）**：纯动量 probe 必须预登记快速失败条件，脉冲死=主动走，硬止损降级为灾难兜底——预期把 probe 类平均亏损近乎砍半

**挂账**：验证新入 probe 仓的 --note 是否带快速失败条件（下轮抽查）；probe 入场侧证据包仍待 1h loop 消费

**下轮重点**：G 节增量 + 快速失败条件落地抽查 + live 平均亏损是否开始收窄
---
