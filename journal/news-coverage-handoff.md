...
- R26 挂的"分页根治截断"**用数据否决**:news tick fetched-count 分布——325 次拉 1 条、60 次 2 条、
  3 次 3 条、1 次 5、1 次 15(那次是重启 backfill);**史上单 tick 最多 15,cap 是 50**。到达率
  平均 0.3 条/180s、最密集 180s 窗口仅 **3 条**。→ 截断(需 >50/tick)天文数字级不可能触发,
  **分页是过度工程,不做**;R26 的 30→50 已远超所需。挂账关闭。
- 回放 36 wake **0 junk-symbol 噪音**;无新漏/噪音。

### ② 覆盖率(健康;1 个"漏"是 rug)
- robinhood ≥100% 榜 5 个,1 个未扫=**VLAD**(0xa1b49ff0…)——**已 rug**:pumped +717% 后流动性抽干到
  **$0**(h6 -100%、FDV $5)。DexPaprika 榜显示的 $237k liq 是**暴涨期陈旧数据**,现已死。→ **不是漏报**
  (正是"崩盘/rug 别当漏报"的情形,safety 层本就该拦)。其余在册,2118 币。clean **胜率 67%** 稳定。
- **自查笔记**:我的健康 grep `FATAL` 撞到了 meme 币名"FATAL/SPCX"(误报 uncaught)——下轮健康检查
  用更精确的模式(如 `uncaughtException|unhandledRejection` 或带前缀),别撞 token 名。

### ③ 安全(无新)/ ④ 健康(绿)
- src/news 无泄漏。tsc 干净、npm test **319 passed**、monitor 存活(pid 45911)、BN 未复现、
  **无真 uncaught**(FATAL 是币名误报)。

**下轮重点:** ① 18932/GLM/BGBTC/BSC ~09-06 TTL 清零复查。② labeled 新激进 wake 成熟后胜率。
③ 健康 grep 别撞 token 名(用精确模式)。④ 守 robinhood 覆盖(rug/liq=0 别当漏报)。
